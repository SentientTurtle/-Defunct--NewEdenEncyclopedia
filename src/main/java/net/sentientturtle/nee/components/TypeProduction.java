...
public class TypeProduction extends Component {
    ...
    public TypeProduction(Type type) {
        this.type = type;
    }
    ...
    public String buildHTML(DataSupplier dataSupplier) {
        ...
        dataSupplier.getProductActivityMap().get(type.typeID)
                .forEach(activity -> html.append("<br><div class='production_blueprint'><img src='").append(ResourceLocation.iconOfTypeID(activity.bpTypeID, dataSupplier)).append("' height='64' width='64'>")
                        .append("<b class='production_link head_font'>").append(new PageReference(dataSupplier.getTypes().get(activity.bpTypeID).name, PageType.TYPE))
